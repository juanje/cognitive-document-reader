...
class TestConceptFiltering:
    ...
    async def test_concept_filtering_prioritizes_cross_section_concepts(self):
        ...
        async with LLMClient(self.config) as llm_client:
            ...
            filtered = synthesizer._filter_concepts_for_glossary(all_concepts, concept_to_sections)
    ...
    def test_concept_filtering_respects_size_limits(self):
        ...
        filtered = synthesizer._filter_concepts_for_glossary(many_concepts, concept_to_sections)
    ...
    def test_concept_filtering_maintains_minimum_concepts(self):
        ...
        filtered = synthesizer._filter_concepts_for_glossary(small_concepts, concept_to_sections)
    ...
    def test_concept_filtering_uses_configurable_parameters(self):
        ...
        filtered = synthesizer._filter_concepts_for_glossary(concepts, concept_to_sections)

        # Should respect the custom max limit
        assert len(filtered) <= config.max_glossary_concepts
        assert len(filtered) == 5  # Should be exactly the max since we have enough concepts
