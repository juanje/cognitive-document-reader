...
class ConceptDefinition(BaseModel):
    ...
    concept_id: str = Field(
        description="Unique identifier (e.g., 'sedentarismo', 'movimiento_natural')"
    )
    ...
    first_mentioned_in: str = Field(
        description="Section ID where this concept was first identified"
    )
    relevant_sections: list[str] = Field(
        default_factory=list, description="Section IDs where concept is relevant"
    )
