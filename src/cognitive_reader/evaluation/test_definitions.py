...
class ConceptualIntegrityTest(SemanticTest):
    ...
    def test_empathy_redefinition(self, knowledge_data: dict[str, Any]) -> TestOutcome:
        ...
        physical_terms = ["physical", "físico", "harmonic coupling", "acoplamiento armónico"]
        ...
        if empathy_concept and ("resonance" in empathy_concept or "resonancia" in empathy_concept):
            score += 0.2
            evidence_parts.append(f"Concept definition includes resonance: {empathy_concept[:100]}...")

        result = TestResult.PASS if score >= 0.7 else (TestResult.PARTIAL if score >= 0.3 else TestResult.FAIL)
        ...
        return TestOutcome(
            ...
            found_evidence="; ".join(evidence_parts) if evidence_parts else "No evidence found",
            failure_reason=None if result == TestResult.PASS else "Missing key empathic resonance terminology"
        )

    def test_psychopathology_redefinition(self, knowledge_data: dict[str, Any]) -> TestOutcome:
        """Test if psychopathologies are redefined as tuning states."""
        all_text = self._extract_summaries_text(knowledge_data)

        # Look for the redefinition terms
        tuning_terms = ["non-standard tuning", "sintonización no estándar", "tuning states"]
        harmonic_terms = ["harmonic dissonance", "disonancia armónica", "frequency deviation"]
        ...
        result = TestResult.PASS if score >= 0.6 else (TestResult.PARTIAL if score >= 0.3 else TestResult.FAIL)
        ...
        return TestOutcome(
            ...
            found_evidence="; ".join(evidence_parts) if evidence_parts else "No evidence found",
            failure_reason=None if result == TestResult.PASS else "Missing tuning state redefinition"
        )
    ...
    def evaluate(self, knowledge_data: dict[str, Any]) -> TestOutcome:
        ...
        combined_result = TestResult.PASS if combined_score >= 0.7 else (
            TestResult.PARTIAL if combined_score >= 0.4 else TestResult.FAIL
        )
        ...
        return TestOutcome(
            ...
            failure_reason=None if combined_result == TestResult.PASS else "One or more concept redefinitions missing"
        )
...
class MetaphorFidelityTest(SemanticTest):
    ...
    def test_prism_metaphor(self, knowledge_data: dict[str, Any]) -> TestOutcome:
        ...
        prism_terms = ["prism", "prisma", "white light", "luz blanca", "spectrum", "espectro"]
        ...
        return TestOutcome(
            ...
            found_evidence="; ".join(evidence_parts) if evidence_parts else "No metaphor evidence found",
            failure_reason=None if result == TestResult.PASS else "Missing prism metaphor elements"
        )
    ...
    def test_tuning_fork_metaphor(self, knowledge_data: dict[str, Any]) -> TestOutcome:
        ...
        fork_terms = ["tuning fork", "diapasón", "vibration", "vibración", "resonance", "resonancia"]
        ...
        result = TestResult.PASS if score >= 0.8 else (TestResult.PARTIAL if score >= 0.4 else TestResult.FAIL)
        ...
        return TestOutcome(
            ...
            found_evidence="; ".join(evidence_parts) if evidence_parts else "No metaphor evidence found",
            failure_reason=None if result == TestResult.PASS else "Missing tuning fork metaphor"
        )
    ...
    def evaluate(self, knowledge_data: dict[str, Any]) -> TestOutcome:
        ...
        return TestOutcome(
            ...
            failure_reason=None if combined_result == TestResult.PASS else "One or more key metaphors missing"
        )
...
class CausalRelationshipTest(SemanticTest):
    ...
    def test_prf_somatic_causality(self, knowledge_data: dict[str, Any]) -> TestOutcome:
        ...
        causal_terms = ["determines", "determina", "causes", "causa", "shapes", "forma", "configures", "configura"]
        prf_terms = ["primordial resonant frequency", "frecuencia resonante primordial", "PRF", "FRP"]
        somatic_terms = ["somatic framework", "entramado somático", "bio-energetic", "bio-energética"]
        ...
        if causal_relationship:
            score += 0.2
            evidence_parts.append("Causal relationship between PRF and somatic framework identified")
        ...
        return TestOutcome(
            ...
            found_evidence="; ".join(evidence_parts) if evidence_parts else "No causal relationship evidence found",
            failure_reason=None if result == TestResult.PASS else "Missing clear PRF → somatic framework causality"
        )

    def test_hierarchical_synthesis(self, knowledge_data: dict[str, Any]) -> TestOutcome:
        ...
        somatic_section = self._find_section_by_title(knowledge_data, ["somatic", "entramado"])
        ...
        synthesis_indicators = [
            "network", "red", "manifests", "manifiesta",
            "bio-energetic", "bio-energética", "consciousness", "conciencia"
        ]

        list_indicators = ["structure", "estructura", "empathy", "empatía", "definition", "definición"]

        found_synthesis = any(term in summary_text.lower() for term in synthesis_indicators)
        ...
        if found_listing and not found_synthesis:
            ...
            evidence_parts.append("WARNING: Contains subsection listing instead of synthesis")
        elif found_listing:
            score += 0.1
        ...
        return TestOutcome(
            ...
            found_evidence="; ".join(evidence_parts) if evidence_parts else "No synthesis evidence",
            failure_reason=None if result == TestResult.PASS else "Missing proper hierarchical synthesis"
        )
    ...
    def evaluate(self, knowledge_data: dict[str, Any]) -> TestOutcome:
        ...
        return TestOutcome(
            ...
            failure_reason=None if combined_result == TestResult.PASS else "Missing causal understanding or synthesis"
        )
...
class NuanceAmbiguityTest(SemanticTest):
    ...
    def test_crystallization_duality(self, knowledge_data: dict[str, Any]) -> TestOutcome:
        """Test if crystallization of self is presented with dual nature."""
        all_text = self._extract_summaries_text(knowledge_data)

        # Look for crystallization concept
        crystal_terms = ["crystallization", "cristalización", "crystallize", "cristaliza"]

        # Look for positive aspects
        positive_terms = ["efficient", "eficiente", "stable", "estable", "necessary", "necesario", "development", "desarrollo"]

        # Look for negative aspects
        negative_terms = ["barrier", "barrera", "rigid", "rígido", "inflexible", "limitation", "limitación"]

        # Look for dual/balanced language
        dual_terms = ["both", "ambos", "dual", "balance", "equilibrio", "two", "dos", "aspect", "aspecto"]
        ...
        return TestOutcome(
            ...
            found_evidence="; ".join(evidence_parts) if evidence_parts else "No duality evidence found",
            failure_reason=None if result == TestResult.PASS else "Missing dual nature of crystallization"
        )

    def test_observer_paradox_unresolved(self, knowledge_data: dict[str, Any]) -> TestOutcome:
        """Test if observer paradox is presented as unresolved."""
        all_text = self._extract_summaries_text(knowledge_data)

        # Look for the paradox
        paradox_terms = ["observer paradox", "paradoja del observador", "dissonant observer", "observador disonante"]

        # Look for resolution language (negative indicators)
        resolved_terms = ["resolved", "resuelta", "solved", "solucionada", "answer", "respuesta", "solution", "solución"]

        # Look for unresolved/speculative language (positive indicators)
        unresolved_terms = ["unresolved", "no resuelta", "controversial", "controvertida", "speculative", "especulativa", "potential", "potencial"]
        ...
        return TestOutcome(
            ...
            found_evidence="; ".join(evidence_parts) if evidence_parts else "No paradox evidence found",
            failure_reason=None if result == TestResult.PASS else "Missing unresolved nature of paradox"
        )
    ...
    def evaluate(self, knowledge_data: dict[str, Any]) -> TestOutcome:
        ...
        combined_score = sum(test.score for test in evaluable_tests) / len(evaluable_tests)
        combined_result = TestResult.PASS if combined_score >= 0.7 else (
            TestResult.PARTIAL if combined_score >= 0.4 else TestResult.FAIL
        )
        ...
        return TestOutcome(
            ...
            failure_reason=None if combined_result == TestResult.PASS else "Missing nuanced understanding"
        )
